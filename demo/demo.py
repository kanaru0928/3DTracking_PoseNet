...
class Args:
    def __init__(self, gpu='0', test_epoch='24') -> None:
        self.gpu_ids = gpu
...
class Posenet:
    ...
    def get_pose(self, original_img, bbox_list, root_depth_list, vis=False):
        ...
        if vis:
            ...
            vis_3d_multiple_skeleton(vis_kps, np.ones_like(vis_kps), skeleton, '3D pose (x,y,z: mm.)')
        return np.array(output_pose_3d_list)
...
def posenet():
    ...
    img = cv2.imread('./media/input.jpg')
    ...
    pose_factory = Posenet(Args())
    pose_factory.get_pose(img, bbox_list, root_depth_list, vis=True)
